...
function fetchLinks() {
	fetch('/_/list', {
		...
		.then(data => {
			// Filter out links without a url
			const filteredData = data.filter(item => item.url);

			// Show admin panel
			document.querySelector('.center-container > div > p').textContent =
				`manage ${filteredData.length} links to ${new Set(filteredData.filter(item => new URL(item.url, location.origin).origin !== location.origin || item.url.startsWith('_')).map(item => item.url)).size} destinations`;
