...
app.use('*', async (c, next) => {
	c.res.headers.set('x-powered-by', 'rinko');
	await next();
});
...
app.get('/*', async (c) => {
	...
	if (destinationUrl === null) {
		const fourZeroFourLink = await c.env.LINKS.get("_404")
		if (!fourZeroFourLink) return c.html(fourZeroFourPage, 404)
		return c.redirect(fourZeroFourLink)
	}
